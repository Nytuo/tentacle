...
import {
  ArrowDownToLine, ArrowUpFromLine, RotateCcw, RefreshCw,
  Moon, Sun, Plus, GitBranch, AlertCircle, ChevronDown, Check,
  GitMerge, CircleX, Loader2, Archive, ArchiveRestore,
} from "lucide-react";
import { open } from "@tauri-apps/plugin-dialog";
import { cn } from "@/lib/utils";
import type { MergeStatus } from "@/lib/api";

/** Compact merge-status pill shown in the toolbar */
function MergeStatusPill({ ms, loading }: { ms: MergeStatus | null; loading: boolean }) {
  if (loading) {
    return (
  ...
  if (ms.status === "up_to_date" && ms.ahead === 0 && ms.behind === 0) return null;
  ...
  switch (ms.status) {
    ...
    case "conflicts":
      ...
      label = "Conflicts";
      pillCls = "bg-destructive/15 text-destructive border border-destructive/30";
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div className={cn("flex items-center gap-1.5 px-2.5 h-7 rounded-md text-xs font-medium cursor-default select-none", pillCls)}>
          {icon}
          <span>{label}</span>
          {aheadBehind && (
            <span className="opacity-70 font-mono text-[10px]">{aheadBehind}</span>
          )}
        </div>
      </TooltipTrigger>
        ...
        <div className="space-y-1 text-xs">
          ...
          {(ms.ahead > 0 || ms.behind > 0) && (
            <p className="text-muted-foreground">
              {ms.source_branch} is {ms.ahead > 0 ? `${ms.ahead} ahead` : ""}{ms.ahead > 0 && ms.behind > 0 ? ", " : ""}{ms.behind > 0 ? `${ms.behind} behind` : ""} {ms.target_branch}
            </p>
          )}
        </div>
      </TooltipContent>
...
export function Toolbar() {
  const { state, tab, dispatch, fetchRemote, pushRemote, pullRemote, refreshAll, refreshDiffs, openRepository, checkoutBranch, stashChanges, popStash } = useGit();
  ...
  return (
    <div className="h-11 border-b flex items-center px-3 gap-1.5 shrink-0 bg-card">
      ...
      <div className="flex items-center gap-1.5">
        {tab.repo.head_branch && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="min-w-[200px] max-h-80 overflow-y-auto">
              ...
              {localBranches.map(branch => (
                ...
                >
                  <Check className={cn("h-3.5 w-3.5 shrink-0", branch.is_head ? "opacity-100 text-primary" : "opacity-0")} />
                  <span className="flex-1 truncate">{branch.name}</span>
                  {branch.ahead != null && branch.ahead > 0 && (
                    <span className="text-[10px] text-muted-foreground tabular-nums">↑{branch.ahead}</span>
                  )}
                  {branch.behind != null && branch.behind > 0 && (
                    <span className="text-[10px] text-muted-foreground tabular-nums">↓{branch.behind}</span>
                  )}
                </DropdownMenuItem>
              ))}
              {localBranches.length === 0 && (
                <div className="px-2 py-3 text-xs text-muted-foreground text-center">No branches</div>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        )}
        ...
        <MergeStatusPill ms={tab.mergeStatus ?? null} loading={isMergeChecking} />
      ...
      <div className="flex items-center">
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              onClick={() => fetchRemote()}
              className={cn("gap-1.5 h-8 text-xs px-2.5", isLoading("fetch") && "opacity-60")}
            >
              <ArrowDownToLine className={cn("h-3.5 w-3.5", isLoading("fetch") && "animate-pulse")} />
              Fetch
            </Button>
          </TooltipTrigger>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <RotateCcw className={cn("h-3.5 w-3.5", isLoading("pull") && "animate-spin")} />
              Pull
            </Button>
          </TooltipTrigger>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="gap-1.5 h-8 text-xs px-2.5"
            >
              <ArrowUpFromLine className={cn("h-3.5 w-3.5", isLoading("push") && "animate-pulse")} />
              Push
            </Button>
          </TooltipTrigger>
          <TooltipContent side="bottom">Push to remote</TooltipContent>
        </Tooltip>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="gap-1.5 h-8 text-xs px-2.5"
            >
              <Archive className={cn("h-3.5 w-3.5", isLoading("stash") && "animate-pulse")} />
              Stash
            </Button>
          </TooltipTrigger>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              className="gap-1.5 h-8 text-xs px-2.5"
            >
              <ArchiveRestore className={cn("h-3.5 w-3.5", isLoading("stash") && "animate-pulse")} />
              Pop
            </Button>
          </TooltipTrigger>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              onClick={async () => { await refreshAll(); await refreshDiffs(); }}
              className="gap-1.5 h-8 text-xs px-2.5"
            >
      ...
      <div className="flex items-center gap-0.5">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={handleOpenRepo}>
              <Plus className="h-4 w-4" />
            </Button>
          </TooltipTrigger>
          <TooltipContent side="bottom">Open repository</TooltipContent>
        </Tooltip>
      </div>
    </div>
  );
