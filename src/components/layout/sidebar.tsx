...
import {
  GitGraph, FileDiff, GitBranch, GitPullRequest,
  Globe, Settings, Tag, Archive, ChevronDown, ChevronRight,
  GitMerge, Check, Trash2, AlertTriangle,
} from "lucide-react";
...
const mergeToolItem: NavItem = {
  id: "merge-tool", label: "Merge Tool", icon: AlertTriangle,
};

function SectionHeader({
  open, onToggle, icon: Icon, label, count,
}: {
  open: boolean;
...
export function Sidebar() {
  const { tab, dispatch, checkoutBranch, mergeBranch, refreshAll, setError } = useGit();
  ...
  const handleDeleteBranch = async (name: string) => {
    try { await api.deleteBranch(name); await refreshAll(); }
    catch (e) { setError(String(e)); }
  };
  const handleApplyStash = async (index: number) => {
    try { await api.applyStash(index, false); await refreshAll(); }
    catch (e) { setError(String(e)); }
  };
  const handleDropStash = async (index: number) => {
    try { await api.dropStash(index); await refreshAll(); }
    catch (e) { setError(String(e)); }
  };
  const handleDeleteTag = async (name: string) => {
    try { await api.deleteTag(name); await refreshAll(); }
    catch (e) { setError(String(e)); }
  };

  const conflictCount = tab.conflictFiles?.length ?? 0;
  const allNavItems = conflictCount > 0
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-card">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 shrink-0">
        <img src="/logo.png" alt="Tentacle" className="h-6 w-6 object-contain shrink-0" />
        <span className="text-sm font-semibold tracking-tight truncate">Tentacle</span>
      </div>
          ...
          return (
            ...
            >
              ...
              {item.id === "changes" && changeCount > 0 && (
                <Badge variant="secondary" className="h-5 min-w-5 px-1.5 text-[10px] font-mono tabular-nums">
                  {changeCount}
                </Badge>
              )}
      ...
      <ScrollArea className="flex-1 min-h-0">
        <div className="px-1.5 py-1 space-y-px">
          ...
          <Collapsible open={branchesOpen} onOpenChange={setBranchesOpen}>
            <SectionHeader
              open={branchesOpen} onToggle={setBranchesOpen}
              icon={GitBranch} label="Local" count={localBranches.length}
            />
            <CollapsibleContent>
              {localBranches.map((branch) => (
                <ContextMenu key={branch.name}>
                  <ContextMenuTrigger asChild>
                    <button
                      onDoubleClick={() => !branch.is_head && checkoutBranch(branch.name)}
                      className={cn(
                    ...
                    >
                      ...
                      <span className="flex gap-0.5 shrink-0">
                        {(branch.ahead  ?? 0) > 0 && (
                          <span className="text-[10px] text-green-500 tabular-nums">↑{branch.ahead}</span>
                        )}
                        {(branch.behind ?? 0) > 0 && (
                          <span className="text-[10px] text-orange-400 tabular-nums">↓{branch.behind}</span>
                        )}
                      </span>
                    </button>
                  </ContextMenuTrigger>
                  <ContextMenuContent>
                    {!branch.is_head && <>
                      <ContextMenuItem onClick={() => checkoutBranch(branch.name)}>
          ...
          {remoteBranches.length > 0 && (
            <Collapsible open={remoteOpen} onOpenChange={setRemoteOpen}>
              <SectionHeader
                open={remoteOpen} onToggle={setRemoteOpen}
                icon={Globe} label="Remote" count={remoteBranches.length}
              />
              <CollapsibleContent>
                {remoteBranches.map((b) => (
                  <div
                    key={b.name}
                    className="flex items-center pl-6 pr-2 h-7 rounded-md text-xs font-mono text-muted-foreground truncate hover:bg-accent/60 hover:text-accent-foreground transition-colors"
                  >
                    {b.name}
                  </div>
                ))}
          ...
          {tab.stashes.length > 0 && (
            <Collapsible open={stashesOpen} onOpenChange={setStashesOpen}>
              <SectionHeader
                open={stashesOpen} onToggle={setStashesOpen}
                icon={Archive} label="Stashes" count={tab.stashes.length}
              />
              <CollapsibleContent>
                {tab.stashes.map((stash) => (
                  <ContextMenu key={stash.index}>
                    <ContextMenuTrigger asChild>
                      <div className="flex items-center pl-6 pr-2 h-7 rounded-md text-xs text-muted-foreground truncate
                                      cursor-default hover:bg-accent/60 hover:text-accent-foreground transition-colors">
                        {stash.message || `stash@{${stash.index}}`}
                      </div>
                    </ContextMenuTrigger>
                    <ContextMenuContent>
                      <ContextMenuItem onClick={() => handleApplyStash(stash.index)}>
                        Apply
                      </ContextMenuItem>
                      <ContextMenuSeparator />
                      <ContextMenuItem
          ...
          {tab.tags.length > 0 && (
            <Collapsible open={tagsOpen} onOpenChange={setTagsOpen}>
              <SectionHeader
                open={tagsOpen} onToggle={setTagsOpen}
                icon={Tag} label="Tags" count={tab.tags.length}
              />
              <CollapsibleContent>
                {tab.tags.map((tag) => (
                  <ContextMenu key={tag.name}>
                    <ContextMenuTrigger asChild>
                      <div className="flex items-center pl-6 pr-2 h-7 rounded-md text-xs font-mono text-muted-foreground truncate
                                      cursor-default hover:bg-accent/60 hover:text-accent-foreground transition-colors">
                        {tag.name}
                      </div>
                    </ContextMenuTrigger>
          ...
          {tab.remotes.length > 0 && (
            <Collapsible open={remotesOpen} onOpenChange={setRemotesOpen}>
              <SectionHeader
                open={remotesOpen} onToggle={setRemotesOpen}
                icon={Globe} label="Remotes" count={tab.remotes.length}
              />
              <CollapsibleContent>
                {tab.remotes.map((remote) => (
                  <div key={remote.name} className="pl-6 pr-2 py-1.5 rounded-md">
                    <div className="text-xs font-medium text-foreground">{remote.name}</div>
