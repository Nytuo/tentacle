...
import {
  GitBranch, Plus, MoreHorizontal, Trash2, GitMerge,
          ...
          <DialogContent>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {localBranches.filter(b => !b.is_head).map(b => (
                  <Button
                    key={b.name}
                    variant={rebaseTarget === b.name ? "secondary" : "outline"}
                    size="sm"
            ...
            {filteredLocal.map((branch) => (
              ...
              >
                ...
                {branch.ahead != null && branch.ahead > 0 && (
                  <span className="text-xs text-added tabular-nums">+{branch.ahead}</span>
                )}
                {branch.behind != null && branch.behind > 0 && (
                  <span className="text-xs text-deleted tabular-nums">-{branch.behind}</span>
                )}
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuLabel className="text-xs font-mono">{branch.name}</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    {!branch.is_head && (
                      <DropdownMenuItem onClick={() => checkoutBranch(branch.name)}>
                        <Check className="h-3 w-3 mr-2" /> Checkout
                      </DropdownMenuItem>
                    )}
                    {!branch.is_head && (
                      <DropdownMenuItem onClick={() => mergeBranch(branch.name)}>
                    ...
                    {!branch.is_head && (
                      <>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleDeleteBranch(branch.name)} className="text-destructive">
